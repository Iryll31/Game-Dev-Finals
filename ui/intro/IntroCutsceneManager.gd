...
const INTRO_BGM_PATH := "res://assets/deuslower-atmosphere-dark-fantasy-dungeon-synth-248210.mp3"
...
func _process(delta: float) -> void:
	...
	_sync_dialogue_to_voice_time(_fallback_elapsed)
	var final_cue_time := float(_intro_cues[_intro_cues.size() - 1]["time"])
	if _fallback_elapsed >= final_cue_time + final_hold_time and final_hold_timer.is_stopped():
		final_hold_timer.start(0.1)
