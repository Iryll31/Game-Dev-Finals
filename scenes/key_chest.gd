...
const KEY_SCENE: PackedScene = preload("res://scenes/Key.tscn")
...
func _spawn_key() -> void:
	...
	key.set("spawn_from_chest", true)
	get_tree().current_scene.add_child(key)
	key.global_position = _loot_point.global_position
	key.call("setup_from_chest", key_id)


func _is_key_already_collected() -> bool:
	return GameManager.is_key_collected(key_id)
