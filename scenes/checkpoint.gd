...
func _process(_delta: float) -> void:
	if spawn_marker != null and _player_nearby and not _activated and Input.is_action_just_pressed("interact"):
...
func _refresh_activated_state() -> void:
	...
	_scene_path = _get_scene_path()
	_activated = GameManager.has_checkpoint_for_scene(_scene_path) and GameManager.checkpoint_pos.distance_to(spawn_marker.global_position) < 1.0
	if _activated:
		_hide_prompt()
...
func _show_checkpoint_text() -> void:
	var label := Label.new()
	label.text = "Checkpoint"
	label.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	label.position = Vector2(-54, -30)
	label.size = Vector2(140, 20)
	label.z_index = 20
	add_child(label)

	var tween := create_tween()
	tween.tween_property(label, "position:y", label.position.y - 12.0, 0.8)
