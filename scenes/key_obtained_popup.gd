...
static func show_popup(key_id: int = 0) -> void:
	var tree := Engine.get_main_loop() as SceneTree
	if tree == null:
		return
	var popup := KeyObtainedPopup.new()
	popup._key_id = key_id
...
func _build_popup_message() -> String:
	var info: Dictionary = GameManager.get_key_unlock_info(_key_id)
	if info.is_empty():
		return (
			"[center]"
	...
	return (
		"[center]"
		+ "[font_size=26][b]Key obtained[/b][/font_size]\n"
		+ "[font_size=17]You obtained [b]%s[/b].[/font_size]\n"
		+ "[font_size=15]This key unlocks [b]%s[/b].[/font_size]"
		+ "[/center]"
	) % [info["key_name"], info["door_name"]]
...
func _build_ui(parent: Control) -> void:
	...
	rich.text = _build_popup_message()
