extends CanvasLayer
class_name CheckpointMessagePopup
...
const DISPLAY_SECONDS: float = 2.2

var _title: String = ""
var _message: String = ""


static func show_message(message: String, title: String = "Checkpoint") -> void:
	var tree := Engine.get_main_loop() as SceneTree
	if tree == null:
		return
	var popup := CheckpointMessagePopup.new()
	popup._title = title
...
func _build_ui(parent: Control) -> void:
	...
	rich.text = (
		"[center][font_size=24][b]%s[/b][/font_size]\n"
		% _title
		+ "[font_size=16]%s[/font_size][/center]" % _message
	)
